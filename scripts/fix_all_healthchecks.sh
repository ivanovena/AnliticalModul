...
echo "=== Fixing all container healthchecks and services ==="

# Fix the streaming service first
echo "Fixing streaming service..."
cat > ./services/streaming/custom_models.py << 'EOF'
import numpy as np
...
# Create healthcheck file for frontend
echo "Fixing frontend healthcheck..."
mkdir -p ./web/public
echo "OK" > ./web/public/healthcheck.html

# Modify docker-compose.yml file for healthchecks
echo "Updating docker-compose.yml with fixed healthchecks..."
sed -i'.bak' 's/python -c "import requests; requests.get('\''http:\/\/localhost:8000\/health'\'')" || exit 1/python -c "import requests; requests.get('\''http:\/\/localhost:8080\/health'\'')" || exit 1/' docker-compose.yml
sed -i'.bak' 's/python -c "import requests; requests.get('\''http:\/\/localhost:8001\/health'\'')" || exit 1/python -c "import requests; requests.get('\''http:\/\/localhost:8090\/health'\'')" || exit 1/' docker-compose.yml

echo "Rebuilding and restarting containers..."
docker-compose down
docker-compose up -d

echo "Done! All healthchecks and services should be fixed now."
